...
Recibe sus percepciones  individuales y
...
Los datos son divididos en dos conjuntos, uno
``público'', el cual  es compartido con los demás agentes del equipo,
y uno ``privado''. 

La sección pública de  datos es compartida a través
...
Este módulo es consultado en cada  iteración
para obtener la próxima acción a ser ejecutada. 
...
la base de conocimiento puede ser actualizada convenientemente. 
...
Cada uno de los
datos de interés es representado mediante un  término diferente. 

 En
muchos casos, esta clase de términos incluyen un parámetro ligado al
número de turno en el cual el dato fue percibido. 
...
Este tipo de creencias es empleado
...
Dado que no constituye información
...
Si la intención no
posee argumentos, entonces es  representada mediante un átomo. 

En otro
caso, se emplea un functor que denota el  nombre de la intención,
acompañado por un argumento. 

Las acciones, por el contrario,  son
representadas a través de listas. 

El primer elemento de la lista es un
atómo denotando  el tipo de acción. 

El resto de la lista contiene,
ocasionalmente, un término que  indica el argumento de la acción, como
por ejemplo el nombre de un nodo o un agente. 
...
Esta es una  secuencia que se reitera en
...
involucrados cuando se cuenta con una  meta ya planificada. 
...
Por esta razón, son las primeras en ser
calculadas y almacenadas durante  el seteo de creencias. 
...
y la posterior selección del más beneficioso. 

 Dichos deseos surgen de
...
Para esto se analizan distintas
condiciones como, por ejemplo, la distancia a un nodo  que no ha sido
explorado. 

Si el nodo se encuentra a una distancia que supera una cota
pre-establecida, entonces el deseo de explorar ese nodo no es
contemplado. 

Los deseos e instanciaciones considerados factibles son
...
Dado que en estos turnos no  es
