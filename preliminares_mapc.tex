...
programación de Sistemas Multi-Agente. Para ello, la competencia
...
a explorar lenguajes, plataformas y herramientas de programación para
...
representado mediante un grafo. Cada nodo del grafo es una locación
válida (y tiene un valor determinado), y existen arcos (con diferente
costo de energía) que permiten a un agente desplazarse de una locación
a otra.

En cada ronda de la competición participan dos equipos rivales. Cada
...
estado de la simulación. El objetivo del juego es obtener la mayor
cantidad de puntos posibles cuando la simulación termina.
...
diferentes locaciones de manera estratégica. La predominancia de un
...
de los pasos de la simulación. Algunas otras situaciones, como el
...
enemigo se encuentra en el mismo nodo. El ataque, de tener éxito,
decrementa la energía del agente enemigo, pudiendo deshabilitarlo en
caso de que ésta llegue a 0.

\item parry: acción disponible únicamente para los
\textit{Reparadores}, \textit{Saboteadores} y \textit{Sentinelas}. La
acción protege al agente de los ataques enemigos, impidiendo que éstos
tengan éxito.

\item probe: acción disponible únicamente para los
\textit{Exploradores}. El agente recibe en su próxima percepción el
valor del nodo en el que se encuentra actualmente. Ésta acción no sólo
resulta importante por conocer el valor del nodo, sino que además
permite que, cuando el nodo es conquistado por el equipo, dicho valor
se sume al total de puntos de la zona. Un nodo en el que no se realizó
\textit{probe} suma únicamente 1 punto al valor total de la zona.

\item inspect: acción disponible únicamente para los
\textit{Inspectores}. El inspector recibe en su próxima percepción la
...
de su compañero de equipo X (volviendo a habilitarlo, en caso de que
su Salud fuera 0).
...
acción idéntica a la vez, existe la posibilidad de que dicha
planificación represente un malgasto de tiempo o recursos para los
agentes; en el marco de MAPC 2011, esto ocurre, por ejemplo, cuando
dos agentes cualesquiera tienen como intención
\textit{survey($X_{i}$)}, cuando dos exploradores tienen como
intención \textit{probear($X_{i}$)}, o cuando dos inspectores tienen
como intención \textit{inspect} estando en el mismo nodo.
